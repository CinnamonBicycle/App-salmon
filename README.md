...
# e2e suite — runs in a disposable VM, no root/Docker on this host:
...
unit tests, and the e2e suite too, if a persistent e2e VM is up. It never silently skips e2e: if
one isn't up, it prints exactly what to run, rather than quietly passing without having run it.
...
|---|---|---|
| `just ci` | Everything below, in order — the full gate | e2e step self-skips with instructions if no VM is up |
...
real `sudo`, and a real Postgres container, and needs somewhere real to run those.

**`just e2e-vm-up` + `just e2e-vm-test`** is the only e2e path: boots a disposable QEMU VM once,
...
disk when you're done. This host only ever needs `/dev/kvm` access — `just setup-e2e-vm` gets you
that with sudo used just once, for installing QEMU and joining the standard `kvm` group, the same
one-time step any KVM user needs on a fresh machine regardless of this project. Nothing
App-Salmon-specific ever touches this host. See `docs/DESIGN.md` §8c for the design and, in
particular, the SSH transport's security model (loopback-bound port forwarding, pubkey-only
auth, a host-generated and pinned SSH host key — not trust-on-first-use) — this path has been
confirmed working end to end against a real KVM host, including the arbitrary-uid Postgres path
(§8a).

There used to also be a way to run the e2e suite directly against this host (`just
setup-e2e`/`test-e2e`, no VM). It was removed (`docs/DESIGN.md` §8d) rather than kept as an
unexercised fallback: an earlier one-shot VM variant sat broken through a later refactor without
anyone noticing, precisely because nothing was running it. A path only stays correct if something
keeps exercising it — the VM path above is what does that now.

Run it yourself rather than relying on e2e-only code paths being exercised for the first time
only after a commit and push.
